...
export class EliteApi {
    private baseUrl = "https://elite-schedule-app-28d3b.firebaseio.com/";
    currentTournament: any = {};
    private tourneyData = {};
    constructor(private http: Http) { }
    getTournaments() {
        return this.http.get(this.baseUrl+"/tournaments.json").map(result => result.json());
    }
    getTournamentData(ID, forceRefresh: boolean = false): Observable<any> {
        ...
        return this.http.get(this.baseUrl+'tournaments-data/'+ID+'.json').map(result => {
